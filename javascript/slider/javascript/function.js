...
function slide(){
	...
	for(var j = 0; j < listNameOfImg.length ;j++){
		if (j == search()){
			list_button += '<button class="index index_select" type="button">' + j + '</button>';
		} else {
			list_button += '<button class="index"type="button" onclick="show_img_index(' + j + ')">' + j + '</button>';
		}
	}	
...
function search(){
	...
	for (; i <= listNameOfImg.length - 1; i++){
		if (imgCurrent == listNameOfImg[i])   return i;
	}
	return -1;
}
