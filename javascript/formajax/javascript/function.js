...
function checkEmail() {
	...
	if (email.search("@") < 1 || email.lastIndexOf(".") < email.search("@") + 2
			|| email.lastIndexOf(".") + 2 > email.lenght) {
...
function showDate() {
	...
	var dateCurrent = new Date(changeMonth(monthSelected) + " 1, "
			+ (yearSelected + nowYear - 300));
	var dayCurrent = dateCurrent.getDay();
	var date = 1;
	for (var i = 13; i < 55; i++) {
		if (((i - 13) >= dayCurrent)
				&& (date <= sumDateOfMonth(monthSelected, yearSelected))) {
			if (date == nowDate && monthSelected == nowMonth
					&& (yearSelected + nowYear - 300) == nowYear) {
				document.getElementsByClassName("colum-calender")[i]
						.setAttribute("class", "today colum-calender");
			} else {
				document.getElementsByClassName("colum-calender")[i]
						.setAttribute("class", "day colum-calender");
			}
			var strDate = "chooseDate(" + date + ", " + (monthSelected + 1)
					+ ", " + (yearSelected + nowYear - 300) + ")";
			...
			document.getElementsByClassName("colum-calender")[i]
					.setAttributeNode(attrOnclick);
			date++;
		} else {
			document.getElementsByClassName("colum-calender")[i].setAttribute(
					"class", "not-day colum-calender");
